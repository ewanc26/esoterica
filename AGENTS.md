...
- "Collaborative" code is an in-memory change log/merge helper only. It has no AT Protocol transport, signature verification, persistence, or stream watcher despite the module comment implying one; `merge_remote` also applies whatever arrives in receipt order rather than reconciling by timestamp, so an out-of-order remote change can overwrite a newer local edit. `chrono_now`'s calendar arithmetic in `format_timestamp` is unit-tested against an independent reference implementation (`days_from_civil`), so trust it; the module comment's claim of cryptographic signing is aspirational, not implemented.
- AT Protocol publishing creates raw `site.standard.publication`/`document` JSON with server validation unset and supplies only a publication URI rather than a verified strong reference/CID. `build_publication_record`/`build_dictionary_record` in `src/atproto.rs` are pure functions the publishing methods call, kept separate so the record shape is unit-testable without a live PDS session — extend those, not the `async` methods, when the shape needs to change. `list_publications` now follows the `cursor` past the first page instead of stopping at 50, and the dictionary record embeds the lexicon exactly once (an earlier version also wrote it into an unread `textContent` field, doubling every record). There is still no size/blob handling for large lexicons. Validate against current Standard.site lexicons before live writes.
